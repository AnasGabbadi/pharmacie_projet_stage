...
function CategoriesCrud() {
  ...
  const fetchCategories = async () => {
    try {
      setLoading(true);
      const res = await categoriesApi.getCategories({ actif: null }); // ✅ Toutes
  ...
  const handleDeleteClick = async (id) => {
    if (window.confirm("Confirmer la suppression ?")) {
      try {
        await categoriesApi.deleteCategorie(id, localStorage.getItem("token"));
  ...
  const handleClosePopup = () => {
    setPopupOpen(false);
    setEditingCategorie(null);
    fetchCategories(); // ✅ Refresh après create/update
  };
              ...
              renderCell: (params) => (
                <Stack direction="row" spacing={0.5}>
                  <IconButton
                    ...
                    onClick={() => handleViewClick(params.row)} // ✅ DÉFINI
                  >
                  ...
                  <IconButton
                    ...
                    onClick={() => handleEditClick(params.row)} // ✅ DÉFINI
                  >
                    <Edit fontSize="small" />
                  </IconButton>
                  <IconButton
      ...
      >
        ...
        <DialogContent sx={{ p: 3 }}>
          <CategorieForm
            ...
            onClose={handleClosePopup}
            onSuccess={fetchCategories} // ✅ CORRIGÉ : fetchCategories au lieu de refetch
          />
        </DialogContent>
      </Dialog>
